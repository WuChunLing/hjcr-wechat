...
import com.hjcr.wechat.dao.CardDao;
import com.hjcr.wechat.dao.ForeverqrcodeDao;
...
public class QRcodeServiceImpl implements QRcodeService{
	...
	@Autowired
	private ForeverqrcodeDao foreverqrcodeDao;
	...
	public void QRcodemessage(String toUserid, String fromUseropenid) throws WxErrorException, IOException {
		...
		User User = userDao.getUserbyuserid(Userid); // 通过UserId获取User
		...
		if (setHierarchyresult.equals("HierarchyExist")) {
			...
			if (!User.getUserHierarchy().isEmpty()) {
				...
				if (st.length > 2) { // 层级不至于一级
					...
					// 获取代理UserId
					User agentUser = userDao.getUserbyuserid(agentUserid);
	...
	public Template getTemplate() {
		...
		return (Template) templateDao.getTemplatebyConfirm(1);
	...
	public String savaUser(String openid) {
		...
		if (userDao.getUserbyOpenid(openid) == null) { // 判断数据库是否存在用户
	...
	public String setUserHierarchy(int Userid, String fromUseropenid) {
		User touser = userDao.getUserbyuserid(Userid); // 获取发送者的user
		User fromuser = userDao.getUserbyOpenid(fromUseropenid);// 获取关注者的user
	...
	public String creatForeverQrcode(String telephone, HttpServletRequest request) {
		...
		// 通过电话获取永久二维码
		Foreverqrcode foreverqrcode = foreverqrcodeDao.getqrcodebyphone(telephone);
	...
	public String LastQrcode(User user, HttpServletRequest request) throws IOException, WxErrorException {
		...
		foreverqrcodeDao.save(foreverqrcode);
		return qrcodeurl;
	}
	...
	public void sendAgentMessage(String openid, String openname) throws IOException, WxErrorException {
		...
		WxMpService wxService = new WxMpServiceImpl();
		wxService.setWxMpConfigStorage(config);
		Voucher voucher = cardDao.findOne(2);
	...
	public void sendCashMessage(String openid) throws IOException, WxErrorException {
		Voucher voucher = cardDao.findOne(1);
