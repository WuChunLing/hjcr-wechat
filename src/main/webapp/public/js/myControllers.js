...
var updateRoleNameURL = preURL_post + 'updateRoleName';    // 修改 角色名称和角色说明
...
var getMyInfoURL = preURL_get + 'getMyInfo.json'; //获取 用户为xx 的 用户信息
...
hjcr.controller('newQCtrl',function($scope,$http){
  ...
  //确认上传模板
	$scope.submitTemplate = function(){
		if($scope.showQrcode===true && $scope.showToux===true){
...
hjcr.controller('updateQCtrl',function($scope,$http){
	...
	$http.post(sureupdateQrcodeURL,{templateId:sessionStorage.templateId})
		.success(function(response){
			...
			$("#toux").css({
				top:$scope.template.templateHeadImgHigh*820-130,
...
hjcr.controller('myBillCtrl',function($scope,$http){
	...
	$scope.getPage = function(num){
 	...
 	$scope.getRecord = function(){
		$scope.getPageMyBill($scope.currentPage,$scope.id);
	}
